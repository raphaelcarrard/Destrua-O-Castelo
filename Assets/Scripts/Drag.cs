...
public class Drag : MonoBehaviour
{
    ...
    void Update()
    {
        LineUpdate();
        SpringEffect();
        prevVel = pedraRB.linearVelocity;
    ...
    void SpringEffect()
    {
        if (spring != null && GameManager.instance.objetosEmCena > 0)
        {
            if (pedraRB.isKinematic == false)
            {
                if (prevVel.sqrMagnitude > pedraRB.linearVelocity.sqrMagnitude)
                {
                    lineFront.enabled = false;
                    lineBack.enabled = false;
                    Destroy(spring);
                    pedraRB.linearVelocity = prevVel;
                }
    ...
    void DestroiPedra()
    {
        if (pedraRB.linearVelocity.magnitude == 0 && pedraRB.IsSleeping())
        {
